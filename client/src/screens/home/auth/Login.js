         ...
         <div className="flex flex-wrap justify-center">
            ...
            className="w-full sm:w-10/12 md:w-8/12 lg:w-6/12 xl:w-4/12 p-6">
                <form onSubmit={onSubmit} className="bg-white rounded-lg -mt-12 border border-gray-200 p-10">
                    <h1 className="heading mb-5">Giriş Yap</h1>
                    <div className="mb-4">
                        <label htmlFor="email" className="form-label">Kullanıcı Adı</label>
                        <input type="text" name="email" id="email" className={`form-input ${showError(errors, 'email') ? 'border-rose-600 bg-rose-50' : 'border-gray-300 bg-white'}`}  placeholder="Email..." value={state.email} onChange={onChange} />
